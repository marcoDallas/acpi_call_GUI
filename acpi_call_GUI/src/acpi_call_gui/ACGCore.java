...
class ACGCore {
    ...
    void installazione(){
        ...
        try {
            ...
            Logger.getLogger(ACGFrame.class.getName()).log(Level.SEVERE, null, ex);
        ...
        if(checkModule("acpi_call")){
        	...
        	JOptionPane.showMessageDialog(null,"Installation failed. log file:\n/usr/local/bin/acpi_call_GUI/log/install_log","error",JOptionPane.WARNING_MESSAGE);
        }
    }
    /**
    ...
    private boolean checkModule(String module){
    	...
    	try {
			...
			Logger.getLogger(ACGFrame.class.getName()).log(Level.SEVERE, null, e);
			JOptionPane.showMessageDialog(null,"An error occured while launchig script,\nplease try again", "error", JOptionPane.ERROR_MESSAGE);
            return false;
		}
    	return false;
    ...
    private void logs(int process, Process script){
        ...
        if(process==0){ 
            url="/usr/local/bin/acpi_call_GUI/log/install_log"; 
        }
        else{
            if(process==1){ 
                url="/usr/local/bin/acpi_call_GUI/log/deactivate_log"; 
            }
            else{ 
                url="/usr/local/bin/acpi_call_GUI/log/automates_log"; 
            }
        }
        PrintWriter printer;
        try {
